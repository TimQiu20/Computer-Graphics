...
void draw() {
  ...
  for (int i = 0; i < n; i++){
    for (int j = 0 ; j < n; j++){
       beginShape();
       texture(bumps_texture);
       //vertex(-50 - m * i,300 - m * j, -300, i/n, j/n);
       //vertex(-50 - m * (i + 1), 300 - m * j, -300, (i+1)/n, j/n);
       //vertex(-50 - m * (i + 1), 300 - m * (j + 1), -300, (i+1)/n, (j+1)/n);
       //vertex(-50 - m * i, 300 - m * (j + 1), -300, i/n, (j+1)/n);
       //vertex(0 - m * i,0 - m * j, -300, i/n, j/n);
       //vertex(0 - m * (i + 1), 0 - m * j, -300, (i+1)/n, j/n);
       //vertex(0 - m * (i + 1), 0 - m * (j + 1), -300, (i+1)/n, (j+1)/n);
       //vertex(0 - m * i, 0 - m * (j + 1), -300, i/n, (j+1)/n);
       vertex(-300 + i * m, 50 + j * m, -300, i / n, j / n);
       vertex(-300 + (i+1) * m, 50+ j * m, -300, (i+1) / n, j /n);
       vertex(-300 + (i+1) * m, 50 + (j+1) * m, -300, (i+1) / n, (j+1) / n);
       vertex(-300 + i * m, 50 + (j+1) * m, -300, i / n, (j+1) / n);
       endShape();
    }
  }
  popMatrix();
