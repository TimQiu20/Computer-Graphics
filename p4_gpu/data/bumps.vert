...
void main() {
  ...
  vec4 tex = texture2D(texture, vec2(vertTexCoord.x, vertTexCoord.y));
  float intensity = 100 * (tex.r * 0.3 + tex.g * 0.6 + tex.b * 0.1);
  vec4 pos = vertex - vec4(normal * intensity, 0.0);
